...
namespace Botble\WebTool\Commands;
...
class ModuleDetailCommand extends BaseMakeCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cms:plugin:make:module-detail {plugin : The plugin name} {name : Model name} {base : Base model name}';
     ...
     * @var string
     */
    protected $description = 'Create a detail for model inside a plugin';
    ...
    public function handle()
    {
        ...
        $replacements = [
            ...
            'src/Http/Controllers/{Base}Controller.stub',
        ...
        foreach ($replacements as $replacement) {
            $this->line('Add below code into ' . $this->replacementModuleDetail(null,
                    str_replace(base_path(), '', $location) . '/' . $replacement));
            $this->info($this->replacementModuleDetail($replacement));
        }
    ...
    {
        return __DIR__ . '/../../stubs/module-detail';
    }
     ...
     * @return string
     */
    protected function replacementModuleDetail(string $file = null, $content = null): string
    {
        $name = strtolower($this->argument('name'));
